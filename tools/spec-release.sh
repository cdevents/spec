...
Examples:
  spec-release.sh -s -> updates all release references on main to from M.m.0 to M.m+1.0-draft
  spec-release.sh -m -> updates all release references on main to from M.m.0 to M+1.0.0-draft
  ...
  spec-release.sh -v 1.2.3 -> updates all release references on 1.2.3
EOF
}
